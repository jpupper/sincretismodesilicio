...
export class GraphVisualizer {
  ...
  setData(data) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const fixedX = Math.cos(angle) * orbitRadius;
      const fixedY = Math.sin(angle) * orbitRadius;
  ...
  update(dt, now) {
    ...
    // Nodes are completely static relative to the center node (no drift, no movement)
